...
- Group blocks deliberately do not touch `EffectiveAri`. A hotel that wants
  block rooms withheld from transient sale still reduces transient availability
  through its channel manager; the platform does not silently do it for them.
  That is a gap we are choosing knowingly rather than papering over with a
  write into a ledger that is supposed to reflect the source.
