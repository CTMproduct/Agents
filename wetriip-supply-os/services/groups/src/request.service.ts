...
export class RequestService {
  ...
  private async accept(
    ...
    await this.notifyBoth(ctx, updated, policy, 'group.request.accepted', {
    ...
    await this.audit.record({
      ...
      after: { status: 'ACCEPTED', total, verdict: evaluation.verdict },
      reason: message ?? null,
      correlationId: ctx.correlationId,
      ip: ctx.ip,
    });
  ...
  private async decline(ctx: RequestContext, request: any, message?: string | null) {
    ...
    await this.notifyBoth(ctx, request, policy, 'group.request.declined', {
  ...
  private async counter(
    ...
    await this.notifyBoth(ctx, request, policy, 'group.request.countered', {
  ...
  async expire(id: string, reason: string): Promise<void> {
    await this.prisma.groupRequest.updateMany({
      ...
      data: { status: 'EXPIRED', settledAt: new Date(), settledBy: 'system:expiry', notes: reason },
    });
  }
      ...
      await this.prisma.notification
        .create({
          data: {
            ...
            payload: { requestId: r.id } as any,
            status: 'NOT_CONFIGURED',
            requirement: 'Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD and SMTP_FROM.',
            requestId: r.id,
            correlationId: `sweep-${r.id}`,
          },
        })
        .catch(() => undefined);
    }
